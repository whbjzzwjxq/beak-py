...
from risc0_fuzzer.settings import TIMEOUT_PER_BUILD, TIMEOUT_PER_RUN
from risc0_fuzzer.zkvm_project import InstructionProjectGenerator
...
class Risc0BeakFuzzer(FuzzerCore[InstrKind, InjectionKind]):
    ...
    def run(self):
        ...
        try:
            ...
            logger.info(
                f"[risc0:{self.commit_or_branch[:7]}] Iteration {self.run_id}: {num_insts} insts"
            )
            ...
            if any(s.is_failure() for s in build_status_list):
                for s in build_status_list:
                    if s.is_failure():
            ...
            logger.info(
                f"[risc0:{self.commit_or_branch[:7]}] Iteration {self.run_id}: done in {elapsed:.2f}s"
            )
    ...
    def get_outputs_from_record(self, record: Record) -> dict[str, str]:
        ...
        try:
            ...
            logger.error(f"Failed to parse RISC0 output: {e}. Raw: {output}")
            return {"status": "failed"}
        ...
        if success:
            logger.info("Loop1: oracle and executor match.")
        else:
