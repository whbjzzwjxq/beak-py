...
class PicoBeakFuzzer(FuzzerCore[InstrKind, InjectionKind]):
    ...
    def run(self):
        ...
        try:
            ...
            logger.info(f"[pico:{self.commit_or_branch[:7]}] Iteration {self.run_id}: {num_insts} insts")
            ...
            logger.info(f"[pico:{self.commit_or_branch[:7]}] Iteration {self.run_id}: done in {elapsed:.2f}s")
    ...
    def build_project(self) -> list[ExecStatus]:
        # Pico's `pico-vm` enables `strict` by default, which turns warnings (incl. deprecations)
        # into hard errors. `--cap-lints allow` keeps dependencies buildable across upstream churn.
        pico_env = {
            "RUSTFLAGS": "--cap-lints allow",
            # Avoid using the crates.io git index (GitHub TLS can be flaky in CI/sandboxes).
    ...
    def get_outputs_from_record(self, record: Record) -> dict[str, str]:
        ...
        try:
            ...
            logger.error(f"Failed to parse Pico output: {e}. Raw: {output}")
            return {"status": "failed"}
